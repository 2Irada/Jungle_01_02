...
public class DataManager : MonoBehaviour
{
    ...
    public void LoadGameData()
    {
        ...
        if (File.Exists(filePath))
        {
            ...
            //isEyeBall 적용
            List<ColoredObject> _allColoredObject = new List<ColoredObject>();
            _allColoredObject.AddRange(FindObjectsOfType<ColoredObject>());
            if (_allColoredObject.Count != gameData.isEyeBallList.Count)
            {
                Debug.LogWarning("Save data and actual colored object count does not match.");
            }
            for (int i = 0; i < _allColoredObject.Count; i++)
            {
                if (_allColoredObject[i].transform.position.x <= gameData.respawnPoint.x)
                {
                    _allColoredObject[i].startAsEyeball = false;
                    _allColoredObject[i].InitEyeball();
                }
                //_allColoredObject[i].startAsEyeball = gameData.isEyeBallList[i];
                //_allColoredObject[i].InitEyeball();
                //if (_allColoredObject[i].name == "Basic Platform_2_5 (4)123")
                //{
                //    print($"loaded {i}");
                //}
            }
    ...
    public void SaveGameData(Vector2 respawnPoint)
    {
        ...
        //    if (_tempList[i].name == "Basic Platform_2_5 (4)123")
        //    {
        //        print($"saved {i}");
        //    }
        //}
